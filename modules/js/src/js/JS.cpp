...
void JS::loop(double dt)
{
    ...
    for (;;)
    {
        ...
        if (err <= 0)
        {
            if (err < 0)
            {
                pimpl->dcl->log("Error executing a pending job.");
                // js_std_dump_error(ctx1);
            }
            break;
        }else{
            pimpl->dcl->log("Succes executing a pending job.");
        }
    }
...
void JS::eval(const char *buf, size_t bufSize, std::string file)
{
    ...
    pimpl->dcl->logIfError(eval);
    if (!pimpl->dcl->logIfError(eval))
    {
        const char *str = JS_ToCString(pimpl->ctx, eval);
        pimpl->dcl->log("Eval result:");
        pimpl->dcl->log(str);
